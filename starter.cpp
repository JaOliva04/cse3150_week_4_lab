...
void spawn_tile(vector<vector<int>>& board) {
    ...
    int idx = rand() % empty_positions.size();
    int r = empty_positions[idx].first;
    int c = empty_positions[idx].second;
...
bool move_left(vector<vector<int>>& board){
    ...
    for(auto& row : board) {
        ...
        row = merge_row(row);
        row = compress_row(row);
        if (row != oldRow) isDifferent = true;
    }
    return isDifferent;
}
...
bool move_up(vector<vector<int>>& board){
    ...
    for(int i = 0; i < 4; i++) {
        ...
        vector<int> oldColumn = column;

        column = compress_row(column);
        column = merge_row(column);
        column = compress_row(column);

        for (int j = 0; j < 4; j++) board[j][i] = column[j];
        if (column != oldColumn) isDifferent = true;
    }
    return isDifferent;
}
